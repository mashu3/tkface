...
def main():
    ...
    for size in sizes:
        label = tk.Label(
            font_frame, text=f"Font Size {size}", font=("Arial", size)
        )
    ...
    features = [
        (
            "DPI Aware",
            "✓ Enabled" if hasattr(root, "DPI_scaling") else "✗ Disabled",
        ),
        (
            "Corner Rounding",
            "✓ Disabled" if sys.platform.startswith("win") else "N/A",
        ),
        ("Scaling Factor", f"{getattr(root, 'DPI_scaling', 1.0):.2f}x"),
    ]
    for feature, status in features:
        feature_label = tk.Label(
            features_frame, text=f"{feature}: {status}", font=("Arial", 10)
        )
