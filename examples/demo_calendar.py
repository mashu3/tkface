...
class DateEntryDemo:
    ...
    def create_widgets(self):
        ...
        # Title
        title = tk.Label(
            main_frame, text="tkface.calendar DatePicker Demo",
        ...
        type_combo = ttk.Combobox(
            config_row1, textvariable=self.dateentry_type_var,
            values=["DateFrame", "DateEntry"], state="readonly", width=10
        )
        type_combo.pack(side='left', padx=(5, 20))
        type_combo.bind('<<ComboboxSelected>>', lambda e: self.change_dateentry_type())

        # Date format selection
        tk.Label(config_row1, text="Date Format:").pack(side='left')
        format_combo = ttk.Combobox(
            config_row1, textvariable=self.format_var,
        ...
        lang_combo = ttk.Combobox(
            config_row2, textvariable=self.lang_var,
        ...
        theme_combo = ttk.Combobox(
            config_row2, textvariable=self.theme_var,
        ...
        week_combo = ttk.Combobox(
            config_row2, textvariable=self.week_start_var,
        ...
        sunday_combo = ttk.Combobox(
            config_row3, textvariable=self.sunday_color_var,
            values=["lightcoral", "lightpink", "lightblue", "lightgreen",
                   "lightyellow", "lightgray", "gray", "darkgray", "none"],
        ...
        saturday_combo = ttk.Combobox(
            config_row3, textvariable=self.saturday_color_var,
            values=["lightblue", "lightcoral", "lightpink", "lightgreen",
                   "lightyellow", "lightgray", "gray", "darkgray", "none"],
            state="readonly", width=10
        )
        ...
        today_combo = ttk.Combobox(
            config_row4, textvariable=self.today_color_var,
            values=["yellow", "orange", "red", "pink", "lightblue",
                   "lightgreen", "lightgray", "none"],
            state="readonly", width=10
        )
        ...
        week_check = tk.Checkbutton(
            config_row4, text="Show Week Numbers",
        ...
        tk.Label(config_row4, text="Button Text:").pack(side='left', padx=(20, 5))
        ...
        reset_button = tk.Button(
            config_row5, text="Reset to Current Date",
    ...
    def generate_code(self):
        ...
        dateentry_class = (
            "DateEntry" if self.dateentry_type_var.get() == "DateEntry"
            else "DateFrame"
        )
        code_lines = [
            "import tkinter as tk",
        ...
        current_date = datetime.date.today()
        if (initial_date and (initial_date.year != current_date.year or
                             initial_date.month != current_date.month)):
        ...
        if (self.sunday_color_var.get() != "lightgray" or
                self.saturday_color_var.get() != "lightgray"):
        ...
        if (self.dateentry_type_var.get() == "DateFrame" and
