...
class LanguageManager:
    ...
    def _determine_language(self, lang_code, root):
        ...
        if lang_code == "auto":
            try:
                ...
                self.logger.warning(
                    "Could not determine locale: %s, using 'en' as fallback", e
                )
                lang_code = "en"
        return lang_code
    ...
    def _load_custom_msgcat(self, lang_code, root):
        ...
        try:
            assets_path_full = os.path.join(
                os.path.dirname(__file__), "..", "locales", f"{lang_code}.msg"
            )
            ...
            self.logger.warning(
                "Failed to load custom msgcat file for %s: %s", lang_code, e
            )
    ...
    def get(self, key, root=None, language=None):
        ...
        try:
            ...
            self.logger.debug(
                "Failed to get translation for '%s' in %s: %s", key, lang_code, e
            )
    ...
    def load_msg(self, lang_code, msg_path, root):
        ...
        try:
            ...
            self.logger.warning(
                "Failed to load msg file %s for %s: %s", msg_path, lang_code, e
            )
            return False
        return True
